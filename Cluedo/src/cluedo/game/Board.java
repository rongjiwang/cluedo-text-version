...
/**
 * take a map.txt file and construct a cluedo board
 * 
 * @author rongji wang
 *
 */
public class Board {
	...
	/**
	 * setup rooms, start points and board
	 * 
	 * @param map
	 * @throws IOException
	 */
	...
	public ArrayList<Door> getExitDoors(char c) {
		...
		for (Room r : allRooms) {
			if (c == 'b' && r instanceof BallRoom) {
				...
				// return associate doors
				list = b.getDoors();
				return list;
			} else if (c == 'i' && r instanceof BilliardRoom) {
				BilliardRoom i = (BilliardRoom) r;
				list = i.getDoors();
				return list;
			} else if (c == 'c' && r instanceof Conservatory) {
				Conservatory co = (Conservatory) r;
				list.add(co.getD());
				return list;
			} else if (c == 'd' && r instanceof DiningRoom) {
				DiningRoom d = (DiningRoom) r;
				list = d.getDoors();
				return list;
			} else if (c == 'h' && r instanceof Hall) {
				Hall h = (Hall) r;
				list = h.getDoors();
				return list;
			} else if (c == 'k' && r instanceof Kitchen) {
				Kitchen k = (Kitchen) r;
				list.add(k.getD());
				return list;
			} else if (c == 'l' && r instanceof Library) {
				Library l = (Library) r;
				list = l.getDoors();
				return list;
			} else if (c == 'o' && r instanceof Lounge) {
				Lounge o = (Lounge) r;
				list.add(o.getD());
				return list;
			} else if (c == 's' && r instanceof Study) {
				Study s = (Study) r;
				list.add(s.getD());
				return list;
			}
	...
	/**
	 * detect starting point position and store
	 *
	 * @param board2
	 */
	private void setUpStartPoint(Character[][] board2) {
		...
		for (int i = 0; i < board2.length; i++) {
			for (int j = 0; j < board2[0].length; j++) {
				if (board2[i][j] == '*') {
					// store start position into list
					temp.add(new Position(i, j));
				}
			}
		}
		// clockwise position sort(manually), for further suggestion use
	...
	/**
	 * set up rooms with associate doors and stairs
	 *
	 * @param board2
	 */
	private void setUpRoom(Character[][] board2) {
			...
			for (int j = 0; j < board2[0].length; j++) {
				if (board2[i][j].equals('+')) {
					...
					if (type == 'k') {
						...
						// identify the room contains multiple doors with
						// associate positions
		...
		// setup rooms with multiple doors
	...
	/**
	 * find the knock door position for multiple rooms
	 *
	 * @param x
	 * @param y
	 * @return
	...
	private Position findKnockPosition(int x, int y) {
		for (int i = 0; i < outputBoard.length; i++) {
			for (int j = 0; j < outputBoard[0].length; j++) {
				// '+' symbol surround by room symbol, white space is knock door
				// position
				if (outputBoard[x][y + 1] == ' ') {
					return new Position(x, y + 1);
				} else if (outputBoard[x][y - 1] == ' ') {
					return new Position(x, y - 1);
				} else if (outputBoard[x + 1][y] == ' ') {
